...
func registerCollector() {
	addrs := strings.Split(*codisUrlF, *separator)
	log.Debugf("parse codis addrs is %s", addrs)
	codisCollector, err := collector.NewCodisCollector(collector.CodisURI{URI: addrs}, *namespace)
	if err != nil {
		log.Fatal(err)
	}
